...
class RedeemAPI {
    ...
    function log_loc() {
        ...
        if (isset($_POST["loc_lat"]) && isset($_POST["loc_long"]) ) {
        ...
        $venue_id = createVenueID($loc_lat, $loc_long);
            ...
            $stmt = $this->db->prepare('SELECT devices_inside FROM current_state_venue WHERE venue_id=?');
            $stmt->bind_param("s", $venue_id);
            $stmt->execute();
            $stmt->bind_result($devices_inside);
            while ($stmt->fetch()) {
                break;
            }
            $stmt->close();
            if( $devices_inside > 12) 
                $color = "Redcolor";
            else if($devices_inside > 5) 
                $color = "Greencolor";
            else
                $color = "Bluecolor";      
    
        
            sendResponse(200, $color);
            return true;
        }
